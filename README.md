...
> Install Sure on a NixOS host:
>
> 1. Add to flake inputs: `sure-nix.url = "github:nSimonFR/sure-nix";`
> 2. Import `inputs.sure-nix.nixosModules.sure` and configure:
>    ```nix
>    services.sure = {
>      enable          = true;
>      port            = 13334;
>      environmentFile = "/run/agenix/sure-app-env";
>      databaseUrl     = "postgresql://sure_user@<host>/sure_production";
>      redisUrl        = "redis://<host>:<port>/2";
>    };
>    ```
> 3. Provision `sure_user` + `sure_production` in Postgres (a `services.postgresql.ensureUsers`/`ensureDatabases` block + a one-shot `ALTER USER` service for the password — see nic-os `rpi5/sure.nix` for a working pattern).
> 4. `sudo nixos-rebuild switch --flake .#<host>` — web UI on the configured port.
>
> **Do not change** `package.nix`'s `tailwindcss-ruby` patchelf invocation: `--set-interpreter` + `LD_LIBRARY_PATH` is required on NixOS, `--set-rpath` corrupts the Bun-bundled binary and the server crashes silently at boot.
## License
